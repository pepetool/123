...
export const MeteoraLiqView: FC = ({ }) => {
  ...
  const handleEnter2btnclick = async () => {
    //
    //const iTx = await DLMM.
    if (!wallet || !publicKey) {
      console.log("钱包未连接");
    ...
    try {
      ...
      const baseMint = await getMint(connection, new PublicKey(basekey));
      console.log(baseMint);  //.decimals
      const quoteMint = await getMint(connection, new PublicKey(quotekey));
      console.log(quoteMint);
      //return;

      console.log('lbPair: ', lbPair.toString());   //== lbPair ID
      ...
      //return;
      const totalXAmount = new BN(baseamt * Math.pow(10, baseMint.decimals));
      const totalYAmount = new BN(quoteamt * Math.pow(10, quoteMint.decimals));//totalXAmount.mul(new BN(Number(1)))
      //totalYAmount
      //dlmmPool.removeLiquidity()

      //console.log([totalXAmount, totalYAmount]);
      //return;
      const newBalancePosition = new Keypair();//Keypair.generate();
      const TOTAL_RANGE_INTERVAL = 34; // 10 bins on each side of the active bin    34+34+1=69
      const activeBin = await dlmmPool.getActiveBin();
      console.log(activeBin);
      const minBinId = activeBin.binId - TOTAL_RANGE_INTERVAL;
      const maxBinId = activeBin.binId + TOTAL_RANGE_INTERVAL;

      //---成功  HBTfi2xrJMAg1fhzTK5eFyK9oZypsA6gVKh5U6Zs3iZ4
      //---     DBTonZVJXzetFofnixgsw3JLwbag8Xby7tcMW9PPNiUf
      let iTx = await dlmmPool.initializePositionAndAddLiquidityByStrategy({
        ...
        strategy: {
          maxBinId,
          minBinId,
          strategyType: StrategyType.SpotImBalanced   //不平衡现货
          /**  StrategyType???  啥意思
            SpotOneSide - 单边现货
            CurveOneSide - 单边曲线
            BidAskOneSide - 单边买卖价
            SpotImBalanced - 不平衡现货
            CurveImBalanced - 不平衡曲线
            BidAskImBalanced - 不平衡买卖价
            SpotBalanced - 平衡现货
            CurveBalanced - 平衡曲线
            BidAskBalanced - 平衡买卖价
           */
        }
      })

      //iTx = setPublicGasfee(iTx);
      const money = 0.2;  //收费2  =  0.1  SOL
      ...
      console.log(err);
      notify({ type: "error", message: "错误", description: '交易出错' });
    }
    ...
    <div className="flex flex-col md:hero mx-auto p-1 md:w-full">
      <div className="">
        ...
        <Flex style={{ display: 'flex', marginTop: '6px', alignItems: 'center', justifyContent: 'flex-end' }}>
          ...
          <Switch value={emvMode} onChange={() => { setemvMode(!emvMode); }} />
        </Flex>
        {
          emvMode ?
            <Flex style={{ display: 'flex', marginTop: '6px', alignItems: 'center', justifyContent: 'flex-end' }}>
