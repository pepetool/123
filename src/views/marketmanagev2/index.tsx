...
export const MarketManageV2View: FC = ({ }) => {
          ...
          if (nowDataAcc.checked) {
            try {
              ...
              if (nowDataAcc.mint > 0) {
                ...
                //const dec = keys.baseDecimals;
                const iTx = await getSendiTx(nowDataAcc.ata, CorATA, nowDataAcc.wallet.publicKey, nowDataAcc.mint, nowDataAcc.keys.baseDecimals);
          ...
          try {
            if (buyRadiovalue === 1) {
              ...
              if (nowDataAcc.sol > buyAmount) {
                ...
                await buyFunction(nowDataAcc.keys, buyAmount, 0, nowDataAcc.connect, nowDataAcc.wallet);
              ...
              if ((nowDataAcc.sol - 0.02) > buyAmt) {
                ...
                await buyFunction(nowDataAcc.keys, buyAmt, 0, nowDataAcc.connect, nowDataAcc.wallet);
                await sleep(buySleepVal);
                console.log(`${nowDataAcc.publickey}买入%: ${buyAmount} SOL`);
              } else {
              ...
              //buyAmount
              const buyAmt = getRandomFloat(buyAmount, ranMax);
          ...
          try {
            if (saleRadiovalue === 1) {
              ...
              if (nowDataAcc.mint >= saleAmount) {
                ...
                await saleFunction(nowDataAcc.keys, saleAmount, nowDataAcc.connect, nowDataAcc.wallet);
              ...
              let dec;
              if (nowTagetPool.quoteMint === SOLMint) {
                dec = baseToken.decimals;
              } else {
                dec = quoteToken.decimals;
              }
              ...
              if (nowDataAcc.mint >= saleAmt) {
                if (saleAmt > 0) {
                  ...
                  await saleFunction(nowDataAcc.keys, saleAmt, nowDataAcc.connect, nowDataAcc.wallet);
                  await sleep(saleSleepVal);
  ...
  async function setPoolInfo(poolId) {
    ...
    let PoolInfo;
    try {
      PoolInfo = await GetparsePoolInfo(markconnection, publicKey, poolId);
  ...
  async function buyFunction(keys, amount, minAmount, userconnect, userWallet) {
    //setGasFree(parseFloat(buyGas));
    ...
    if (keys.quoteMint.toString() === SOLMint) {
      const buyAmt = (amount * LAMPORTS_PER_SOL).toFixed(0);
      const tx = await swap_Buy_Wallet(keys, BigInt(buyAmt), BigInt(minAmount), userWallet);
      if (!emvMode) {
      ...
      const buyAmt = (amount * LAMPORTS_PER_SOL).toFixed(0);
      const tx = await swapOut_Buy_wallet(keys, BigInt(buyAmt), BigInt(minAmount), userWallet);
      if (!emvMode) {
        const sent = await userconnect.sendTransaction(tx, [userWallet], { skipPreflight: true })
        console.log("swapped in tx id:", sent);
        //msg_success('交易已提交');
  ...
  async function saleFunction(keys, amount, userconnect, userWallet) {
    setRayPubGas(parseFloat(buyGas));
    addSoftLog(`卖出钱包: ${truncateString(userWallet.publicKey.toString(), 16, 8, 8)} 数量:${amount}`);
    if (keys.quoteMint.toString() === SOLMint) {
      const dec = keys.baseDecimals;
      const saleamtV = Math.floor(amount * Math.pow(10, dec));
      const tx = await swap_Sale_wallet(keys, BigInt(saleamtV), BigInt(0), userWallet);
      //console.log(amount)
      if (!emvMode) {
        const sent = await userconnect.sendTransaction(tx, [userWallet], { skipPreflight: true })
        console.log("swapped in tx id:", sent);
      ...
      const dec = keys.quoteDecimals;
      console.log(dec);
      const saleamtV = Math.floor(amount * Math.pow(10, dec));
      console.log(saleamtV);
      const tx = await swapOut_Sale_wallet(keys, BigInt(saleamtV), BigInt(0), userWallet);
      if (!emvMode) {
        const sent = await userconnect.sendTransaction(tx, [userWallet], { skipPreflight: true })
        console.log("swapped in tx id:", sent)
        //console.log(amount) 
  ...
  const handleSetPool = async () => {
    ...
    if (!checkFromMint) {
      try {
        console.log(poolId);
        const poolInfo = await setPoolInfo(poolId);
        console.log("poolInfo", poolInfo);
        let baseMintA;
        if (nowTagetPool.quoteMint === SOLMint) {
          baseMintA = nowTagetPool.baseMint
          m_Mint=new PublicKey(baseMintA);
          console.log(m_Mint);
        } else if (nowTagetPool.baseMint === SOLMint) {
          baseMintA = nowTagetPool.quoteMint;
          m_Mint=new PublicKey(baseMintA);
          console.log(m_Mint);
        } else {
          notify({ type: "error", message: "错误", description: "当前交易对不支持!" });
          return;
        }
        console.log('setAccKeys');
        const FastDataAcc = dataAccList[0];
        setConnection(markconnection);
        setWallet(FastDataAcc.wallet);
        const cormarketID = new PublicKey(nowTagetPool.marketId);
        let baseDec = nowTagetPool.baseDecimals;
        let quoteDec = nowTagetPool.quoteDecimals;
        //let quoteMint;// = new PublicKey(nowTagetPool.quoteMint);
        // const [baseMint, quoteMint] = [marketInfo.baseMint, marketInfo.quoteMint];
        // if (nowTagetPool.quoteMint === SOLMint) {
        //   baseDec = nowTagetPool.baseDecimals;
        //   quoteDec = nowTagetPool.quoteDecimals;
        //   //quoteMint = new PublicKey(nowTagetPool.quoteMint);
        // } else {
        //   baseDec = nowTagetPool.quoteDecimals;
        //   quoteDec = nowTagetPool.baseDecimals;
        //   //quoteMint = new PublicKey(nowTagetPool.baseMint);
        // }
        // const baseDec = nowTagetPool.baseDecimals;
        // const quoteDec = nowTagetPool.quoteDecimals;
        const marketInfo = await getMarketInfoA(cormarketID)
        const [baseMint, quoteMint] = [marketInfo.baseMint, marketInfo.quoteMint];

        const keys = await getKeys(cormarketID, baseDec, quoteDec);
        ...
        for (let i = 0; i < dataAccList.length; i++) {
          const nowDataAcc = dataAccList[i];
          const [ownerBaseAta, ownerQuoteAta] = await Promise.all([getAta(baseMint, nowDataAcc.wallet.publicKey), getAta(quoteMint, nowDataAcc.wallet.publicKey)]);
          const newKeys = { ...keys };
          newKeys.ownerBaseAta = ownerBaseAta;
          newKeys.ownerQuoteAta = ownerQuoteAta;
          newKeys.wallet = nowDataAcc.wallet.publicKey;
          //console.log(newKeys);
          //console.log(ownerBaseAta.toString());
          //console.log(ownerQuoteAta.toString());
          //console.log(nowDataAcc.wallet.publicKey.toString());
          //console.log('1111111111');
          const updatedData = [...dataAccList];
          updatedData[i].keys = newKeys;
          // updatedData[i].keys.ownerBaseAta = ownerBaseAta;
          // updatedData[i].keys.ownerQuoteAta = ownerQuoteAta;
          // updatedData[i].keys.wallet = nowDataAcc.wallet.publicKey;
          setDataAccList(updatedData);
          //console.log(nowDataAcc.keys);
        }

        setdisTokenAddr(baseMintA);
        const tokenMetaData = await getTokenMetadataProc(markconnection, new PublicKey(baseMintA));
        ...
        notify({ type: "error", message: "设置池子失败", description: "请检查池子ID是否正确" });
        console.log(err);
        setIsLoading(false);
      }
    } else {
      setIsLoading(true);
      try {
        const thepoolInfo: any = await queryLpPair(poolId, "So11111111111111111111111111111111111111112");
        ...
        if (!thepoolInfo) {
          ...
          if (thepoolInfo.Raydium_LiquidityPoolv4.length > 0) {
            if (thepoolInfo.Raydium_LiquidityPoolv4.length <= 1) {
              console.log("只有1个池子");
              const getPoolID = thepoolInfo.Raydium_LiquidityPoolv4[0].pubkey;
              ...
              const getPoolID = highestLpReservePool.pubkey;
              const PoolInfo = await setPoolInfo(getPoolID);
              let baseMintA;
              if (nowTagetPool.quoteMint === SOLMint) {
                baseMintA = nowTagetPool.baseMint
                m_Mint=new PublicKey(baseMintA);
    ...
    <div>
      ...
      <Flex vertical={true} gap={"middle"} >
        <Flex gap={10} justify={"flex-start"} align={"flex-start"}>
          {!canAccounted &&
            <Card style={{ width: 600 }} title="1.私钥填写">
          ...
          {!canPoolInited &&
            <Card style={{ width: 600, height: 384 }} title="2.设置池子信息 - VB">
              {/* <Flex gap={10} justify={"flex-start"} align={"center"}>
              ...
              <Button type="primary" size="large" onClick={handleSetPool}>
                设置池子信息
              </Button>
              <p><Text>VB速度比VA快,VB不使用Raydium SDK 使用了自研算法(极大减少网络RPC访问频率),使用VB版本建议打开MEV模式</Text></p>
        ...
        <Flex gap={"middle"} justify={"flex-start"} align={"flex-start"}>
          <Card style={{ width: 600 }} title="帐号信息">
            <Flex gap={10} justify={"flex-start"} align={"center"}>
              <Popconfirm
                title="一键归集确认"
          ...
          <Card style={{ width: 600 }} title="池子/操作 - VB">
