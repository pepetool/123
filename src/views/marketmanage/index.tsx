...
import { ComputeBudgetProgram, Connection, Keypair, LAMPORTS_PER_SOL, PublicKey, SystemProgram, Transaction, VersionedTransaction } from '@solana/web3.js'
import { useTranslation } from 'next-i18next'
import { ExtensionType, TOKEN_PROGRAM_ID, TokenAccountNotFoundError, TokenInvalidAccountOwnerError, createCloseAccountInstruction, getAccount, getAssociatedTokenAddress, getExtensionData, getExtraAccountMetaAddress, getExtraAccountMetas, getMetadataPointerState, getMint, getMintCloseAuthority, getTokenMetadata, resolveExtraAccountMeta } from '@solana/spl-token';
...
import { DEFAULT_TOKEN, GAS_LEVEL, setConnection, setEmvFee, setEmvLevel, setGasFree, setWallet, sleep } from 'utils/config';
...
export const MarketManageView: FC = ({ }) => {
  ...
  const [emvMode, setemvMode] = useState(false);
        ...
        if (nowDataAcc.checked) {
          try {
            if (saleRadiovalue === 1) {
              ...
              // }
              const buyAmt = (nowDataAcc.mint / 100 * saleAmount);
              if (nowDataAcc.mint >= buyAmt) {
                if (buyAmt > 0) {
                  console.log(buyAmt);
                  await saleproc(nowDataAcc.connect, nowDataAcc.wallet, buyAmt);
                  await sleep(saleSleepVal);
                  console.log(`${nowDataAcc.publickey}卖出%: ${buyAmt} `);
                }
              } else {
          ...
          const promise = (async () => {
            try {
              if (buyRadiovalue === 1) {
                ...
                const buyAmt = (nowDataAcc.sol - 0.02) / 100 * buyAmount;
                if ((nowDataAcc.sol - 0.02) > buyAmt) {
                  //const buyamta = Number(buyAmt.toFixed(0));
                  await buyproc(nowDataAcc.connect, nowDataAcc.wallet, buyAmt);
                  console.log(`${nowDataAcc.publickey}买入%: ${buyAmount} SOL`);
                } else {
                  addSoftLog(`钱包: ${truncateString(nowDataAcc.wallet.publicKey.toString(), 16, 8, 8)} %SOL小于0.02 跳过`)
          ...
          const promise = (async () => {
            try {
              if (saleRadiovalue === 1) {
                ...
                // console.log(buyAmt);
                if (nowDataAcc.mint >= buyAmt && buyAmt > 0) {
                  console.log(buyAmt);
                  await saleproc(nowDataAcc.connect, nowDataAcc.wallet, buyAmt);
                  console.log(`${nowDataAcc.publickey}卖出%: ${buyAmt} `);
                } else {
  ...
  const handleSetPool = async () => {
    ...
    if (!checkFromMint) {
      try {
        ...
        setdisTokenAddr(baseMint);

        const tokenMetaData = await getTokenMetadataProc(markconnection, new PublicKey(baseMint));
        ...
        try {
          ...
          for (let j = 0; j < tokenAccount.length; j++) {
            ...
            if (Tokeninfo.tokenAmount.uiAmount === 0) {
              ...
              const uUnit = ComputeBudgetProgram.setComputeUnitLimit({ units: 0.00005 * LAMPORTS_PER_SOL });
              transaction.add(uUnit);
    ...
    <div>
      ...
      <Flex vertical={true} gap={"middle"} >
        <Flex gap={10} justify={"flex-start"} align={"flex-start"}>
          ...
          {!canPoolInited &&
            <Card style={{ width: 600, height: 384 }} title="2.设置池子信息 - VA">
              {/* <Flex gap={10} justify={"flex-start"} align={"center"}>
        ...
        <Flex gap={"middle"} justify={"flex-start"} align={"flex-start"}>
          <Card style={{ width: 600 }} title="帐号信息">
            <Flex gap={10} justify={"flex-start"} align={"center"}>
              ...
              >
                ...
                <Button size="middle" >一键归集SOL到</Button>
              </Popconfirm>
              <Input id="backaddr" placeholder="接收地址" value={enterBackAddr} onChange={onChangeBackAddr} style={{ width: '50%' }} />
              <Button size="middle" onClick={handleRefAccInfo}>刷新</Button>
              <Button size="middle" onClick={handleToggleAutoRefresh}>{autoRefreshAcc ? '停止' : '自动刷新'}</Button>
            </Flex>
            <Flex style={{ marginTop: 10 }} justify={"flex-start"} align={"center"}>
              <Button size="middle" onClick={handleCloseAcc}>批量回收账户</Button>
            </Flex>
            <Flex gap={10} justify={"flex-start"} align={"center"}>
          ...
          <Card style={{ width: 600 }} title="池子/操作 - VA">
